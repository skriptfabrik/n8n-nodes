...
async function parseJSONBody(
  ...
  if (body.customPlacementConfig) {
    try {
      body.customPlacementConfig = JSON.parse(
        body.customPlacementConfig as string,
      );
    } catch (_error) {
      // Ignore JSON parse error
    }
              ...
              const extractBucketsList = (page: INodeExecutionData) => {
                ...
                if (buckets) {
                  executions = executions.concat(
                    buckets.map((bucket) => ({ json: bucket })),
                  );
                }
              };
  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        default: true,
        placeholder:
          'Whether or not to automatically apply an event based hold to new objects',
      },
      {
        ...
        default: '[]',
        placeholder:
          'Default Access Controls for new objects when no ACL is provided',
      },
      {
      ...
      },
      {
        ...
        default: 'STANDARD',
        placeholder:
          "The bucket's default storage class for objects that don't define one",
      },
      {
      ...
      },
      {
        ...
        default: '{}',
        placeholder:
          "The bucket's website configuration for when it is used to host a website",
      },
    ],
  },
];
