  ...
  do {
    ...
    if (response.statusCode === 409) {
      ...
      if (
        ...
        (requestOptions.body as IDataObject).version = body[0]['version'];
        remainingRetries = 1;
      } else {
        remainingRetries = 0;
      }
...
export async function fulfillmenttoolsApiRequestAllItems<T = unknown>(
  ...
  const returnData: (T & { id?: string })[] = [];

  let responseItems;

  do {
    const responseData = await (fulfillmenttoolsApiRequest<{
      [R: string]: (T & { id?: string })[];
    }>).call(this, method, url, body, qs);

    responseItems = responseData[resource];
