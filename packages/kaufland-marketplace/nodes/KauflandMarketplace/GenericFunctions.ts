...
interface KauflandResponse extends IDataObject {
  data: unknown[];
  pagination?: {
    limit: number;
    offset: number;
...
export async function kauflandMarketplaceRequestAllItems<
  T extends KauflandResponse,
>(
  ...
  if (
    ...
    const remaining = await kauflandMarketplaceRequestAllItems(
      instance,
      requestData,
